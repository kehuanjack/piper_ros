...
class C_PiperRosNode():
    ...
    def __init__(self) -> None:
        ...
        self.delay_test_mode = 'M1'
        if rospy.has_param('~delay_test_mode'):
            self.delay_test_mode = rospy.get_param('~delay_test_mode')
        rospy.loginfo("%s is %s", rospy.resolve_name('~delay_test_mode'), self.delay_test_mode)        
        # publish
    ...
    def PublishCtl(self):
        '''需先运行delay_test节点，再运行本节点'''
        seq = self.armSeq.seq
        seq1_temp = self.armSeq.seq1
        seq2_temp = self.armSeq.seq2
        seq3_temp = self.armSeq.seq3

        # 下发159控制指令 每次夹爪回零主控会记录seq1和seq2，夹爪给予其他非零值会重置主控记录的各阶段seq数
